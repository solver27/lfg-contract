...
const LFGNFTArt = hre.artifacts.require("LFGFireNFT");
...
describe("LFGFireNFT", function () {
  ...
  before("Deploy contract", async function () {
    try {
      [accounts[0], accounts[1], accounts[2], accounts[3], minter] =
        await web3.eth.getAccounts();
      LFGFireNFT = await LFGNFTArt.new();
      await LFGFireNFT.setMinter(minter, true);
    } catch (err) {
      console.log(err);
    }
  });
  ...
  it("test NFT Royalties", async function () {
    ...
    await LFGFireNFT.multiMint([accounts[1], accounts[2], accounts[3]]);
