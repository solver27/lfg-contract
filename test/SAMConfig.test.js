...
describe("SAMConfig", function () {
  ...
  before("Deploy contract", async function () {
    try {
      [owner, burnAddress, revenueAddress] = await web3.eth.getAccounts();
  ...
  it("test set valid fee rate", async function () {
    ...
    console.log("RoyaltiesFeeRate ", royaltyFeeRate.toString());
    ...
    console.log("FeeBurnRate ", feeBurnRate.toString());
  ...
  it("test set valid revenue address", async function () {
    ...
    await expect(
      SAMConfig.setRevenueAddress("0x0000000000000000000000000000000000000000", {from: owner})
  ...
  it("test set valid duration", async function () {
    ...
    await expect(SAMConfig.setMinDuration(15 * 24 * 3600, {from: owner})).to.be.revertedWith(
    ...
    await expect(SAMConfig.setMaxDuration(0.5 * 24 * 3600, {from: owner})).to.be.revertedWith(
