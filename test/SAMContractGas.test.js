...
describe("SAMContractGas", function () {
  ...
  it("test buy now feature", async function () {
    ...
    console.log("Balance of account 2 ", balanceOfAccount2.toString());
    assert.isAbove(parseInt(balanceOfAccount2.toString()), 10001994337036928800000);
  ...
  it("test auction and bidding", async function () {
    ...
    console.log("Escrow tokens of account 4 before place bid", account4Tokens.toString());
    ...
    console.log("Escrow tokens of account 4 after place bid", account4Tokens.toString());

    assert.equal(account4Tokens.toString(), "1200000000000000000");

    let balanceOfAccount4 = await web3.eth.getBalance(accounts[4]);
    assert.isAbove(parseInt(balanceOfAccount4.toString()), 9998709700978160000000);
    ...
    account4Tokens = await SAMContractGas.addrTokens(accounts[4]);
    console.log("Escrow tokens of account 4 after there is higher bid", JSON.stringify(account4Tokens));
    assert.equal(account4Tokens.toString(), "0");
    ...
    await expect(SAMContractGas.claimNft(biddingsOfAddr5[0]["id"], {from: accounts[5]})).to.be.revertedWith(
    ...
    let balanceOfAccount2 = await web3.eth.getBalance(accounts[2]);
    console.log("Balance of account 2 ", balanceOfAccount2.toString());
    assert.isAbove(parseInt(balanceOfAccount2.toString()), 10003494037036928800000);
  ...
  it("test remove listing ", async function () {
    ...
    await expect(SAMContractGas.removeListing(listingId, {from: accounts[2]})).to.be.revertedWith(
    ...
    await expect(SAMContractGas.removeListing(listingId, {from: accounts[1]})).to.be.revertedWith(
  ...
  it("test dutch auction ", async function () {
    ...
    let balanceOfAccount2 = await web3.eth.getBalance(accounts[2]);
    console.log("Balance of account 2 ", balanceOfAccount2.toString());
    assert.isAbove(parseInt(balanceOfAccount2.toString()), 10004493037036928800000);
